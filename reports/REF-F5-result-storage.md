...
| ✅ V-12 | **Revert the V-10 `statusInfo.links` item-link leak.** `GET /jobs/{id}` must only ever carry lifecycle/navigation links (`self`, `results`), never per-output result-reference `href`s — that blurs the OGC API Processes boundary between job status and job results. `_apply_stored_references` (result_storage_coordinator.py) no longer appends a `rel="item"` link per stored output to `status_info.links`; it now *only* populates `job.stored_outputs`, which `GET /jobs/{id}/results` already reads to build the OGC `document` response. `self`/`results` links (added by `execution_steps.py`/`ResultStorageObserver._finalize_publication`) are unaffected. | V-10 |
