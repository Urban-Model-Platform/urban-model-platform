...
async def test_retry_verification_success():
    ...
    mgr, repo, http_client = make_manager(
        post_response, get_responses=get_responses, retry_port=retry_port
    )
    resp = await mgr.run_execution_pipeline("prov:procC", {"inputs": {}}, {})
    job_id = resp["headers"]["Location"].split("/")[-1]
    job = await repo.get(job_id)
    assert job is not None, "Job should exist"
    assert job.status_info is not None, "status_info should be populated"
...
async def test_retry_verification_failure_downgrades_status():
    ...
    retry_port = AlwaysFailRetry(
        attempts=3, wait_initial=0, wait_max=0, exception_types=(Exception,)
    )
    mgr, repo, http_client = make_manager(
        post_response, get_responses=get_responses, retry_port=retry_port
    )
    resp = await mgr.run_execution_pipeline("prov:procD", {"inputs": {}}, {})
    job_id = resp["headers"]["Location"].split("/")[-1]
    job = await repo.get(job_id)
    assert job is not None, "Job should exist"
    assert job.status_info is not None, "status_info should be populated"
